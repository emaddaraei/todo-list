...
export default function Header() {
    ...
    return (
        <>
                        ...
                        <a href="/" className="flex items-center">
                            <div className="flex items-center gap-2">
                                <Image src={logoSrc} alt="Logo" width={36} height={36} />
                                    <h1 style={{ color: 'var(--brand-color)' }} className="text-xl font-bold">مدرن تسک</h1>
                            </div>
                        </a>
                                    ...
                                    {menuItems.map((item) => {
                                        ...
                                        return (
                                            <li key={item.title}>
                                                <a
                                                    href={item.href}
                                                    className={`text-sm ${isActive ? 'font-bold' : ''} ${isActive ? '' : 'text-gray-500 hover:text-gray-900'}`}
                                                    style={isActive ? { color: 'var(--brand-color)' } : {}}
                                                >
                                                    {item.title}
                                                </a>
                                            </li>
            ...
            {menuOpen && (
                <div className="fixed inset-0 bg-black/60 z-[100] md:hidden" onClick={() => setMenuOpen(false)}>
                    <div
